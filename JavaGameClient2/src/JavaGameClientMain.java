	...
	class Myaction implements ActionListener // 내부클래스로 액션 이벤트 처리 클래스
	{
		...
		public void actionPerformed(ActionEvent e) {
			...
			for(int i =0; i<UserList.length; i++) {
				UserList[i] = new User();
				UserList[i].UserName = new String("User "+(i+1));
				//System.out.println("in Main::"+UserList[i].UserName);
			}
			//System.out.println(username);
			JavaGameClientProfileList view = new JavaGameClientProfileList(username, ip_addr, port_no);
